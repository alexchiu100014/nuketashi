...
use std::io::{BufReader, Cursor, Read, Seek, BufRead};
        ...
        match command[0] {
            "$L_CHR" => {
                ...
                if command.len() == 6 {
                    ...
                    let tex = crate::assets::load_image_from_rgba(&img.rgba_buffer,
                        img.metadata.width as usize, img.metadata.height as usize, "___");
            ...
            "$L_MONT" => {
                ...
                for (i, entry) in (&command[7..]).iter().enumerate() {
                    let mut entry: i32 = entry.parse().unwrap();
    ...
    fn draw(&mut self, _v: (f64, f64), c: Context, g: &mut G2d) -> Option<()> {
        ...
        for l in &self.layers {
            for (t, (x, y)) in l.textures.iter().zip(l.origin.iter()) {
                ...
                let t = if let Some(t) = t { t } else { continue; };
